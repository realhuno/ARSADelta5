...
def on_simulate_lap(data):
    ...
    #clientSock.sendto(str(node_index), (UDP_IP_ADDRESS, UDP_PORT_NO))
...
def pass_record_callback(node, ms_since_lap):
    ...
    if RACE.race_status is 1:
        ...
        if last_lap_id is None: # No previous laps, this is the first pass
            ...
            clientSock.sendto(str(node.index)+str("|")+str(int(lap_time)), (UDP_IP_ADDRESS, UDP_PORT_NO))
        ...
        if lap_time > (min_lap * 1000) or lap_id == 0:
            ...
            if lap_id > 2:
                emit_phonetic_text('Pilot ' + str(pilot_id)+ ' finished')
        else:
